...
module.exports.createTrade = async (req, res) => {
    try {
        ...
        const trade = await tradeService.createTrade(req.user, {
            stockName, stockSymbol, quantity, type, entryType, price, date,
        });
...
module.exports.getAllTrades = async (req, res) => {
    try {
        ...
        const data = await tradeService.getAllTrades(req.user._id, page, limit, search, status);
        return res.status(200).json({ success: true, message: 'Fetched trades', ...data });
    } catch (err) {
...
module.exports.closeTrade = async (req, res) => {
    try {
        ...
        const trade = await tradeService.closeTrade(
            req.params.tradeId, closePrice, closeDate, closeQuantity, req.user._id
        );
        
        return res.status(200).json({ success: true, message: 'Trade closed successfully', trade });
    } catch (err) {
...
module.exports.updateTrade = async (req, res) => {
    try {
        const updatedTrade = await tradeService.updateTrade(
            req.params.tradeId, req.body, req.user._id
        );
        return res.status(200).json({ success: true, message: 'Trade updated successfully', trade: updatedTrade });
    } catch (err) {
