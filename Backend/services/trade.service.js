...
async function fetchWithRetry(fn, retries = 2) {
  try {
    ...
    if (retries === 0)
      throw httpError(502, "Failed to fetch price after retries");
...
module.exports.getAllTrades = async (
  ...
  const activeTrades = trades.filter(
    (t) => t.status !== "closed" && t.remainingQty > 0,
  );
...
module.exports.getTrade = async (tradeId, userId) => {
  ...
  if (trade.status !== "closed" && trade.remainingQty > 0) {
    try {
      trade._currentMarketPrice = await module.exports.getStockPrice(
        trade.stockSymbol,
      );
    } catch {
...
module.exports.closeTrade = async (
  tradeId,
  closePrice,
  closeDate,
  closeQuantity,
  ...
  if (exitQty > trade.remainingQty) {
    throw httpError(
      400,
      `Cannot close ${exitQty}. Only ${trade.remainingQty} units remaining.`,
    );
  }
  ...
  trade.avgExitPrice =
    trade.avgExitPrice === null
      ? exitPrice
      : (trade.avgExitPrice * trade.closedQty + exitPrice * exitQty) /
...
module.exports.updateTrade = async (tradeId, tradeData, userId) => {
  ...
  Object.keys(tradeData).forEach((key) => {
    if (!IMMUTABLE.includes(key)) trade[key] = tradeData[key];
  });
...
module.exports.deleteTrade = async (tradeId, userId) => {
  const deleted = await tradeModel.findOneAndDelete({
    _id: tradeId,
    user: userId,
  });
...
module.exports.addPosition = async (tradeId, quantity, price, date, userId) => {
  const trade = await tradeModel.findOne({ _id: tradeId, user: userId });
  ...
  if (trade.status === "closed") {
    throw httpError(400, "Cannot add to a closed trade");
  }
  ...
  if (isNaN(addQty) || addQty < 1) {
    throw httpError(400, "Quantity must be a valid integer greater than 0");
  }

  if (isNaN(addPrice) || addPrice <= 0) {
    throw httpError(400, "Price must be a valid number greater than 0");
  }

  // Long positions can only average by BUY
  if (trade.type === "long" && trade.entryType !== "buy") {
    throw httpError(
      400,
      "Long positions can only be averaged using buy entries",
    );
  }

  // Calculate weighted average entry price
  const newAvgPrice =
    (trade.entryPrice * trade.openQty + addPrice * addQty) /
    (trade.openQty + addQty);

  trade.entryPrice = Number(newAvgPrice.toFixed(2));
  trade.openQty += addQty;

  trade.addCount += 1;
  trade.lastAddedAt = date || new Date();
