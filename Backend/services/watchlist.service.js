...
module.exports.getWatchlist = async (userId) => {
   // ── Get All Watchlists ────────────────────────────────────────────────────────
    ...
    const uniqueSymbols = [
        ...new Set(
            watchlists.flatMap(w => w.stocks.map(s => s.stockSymbol))
        )
    ];
    ...
    await Promise.allSettled(
        uniqueSymbols.map(async symbol => {
            try   { priceMap[symbol] = await tradeService.getStockPrice(symbol); }
...
module.exports.addStocks = async ({ watchlistId, stocks, userId }) => {
    const watchlist = await watchlistModel.findOne({ _id: watchlistId, user: userId });
    if (!watchlist) throw httpError(404, 'Watchlist not found.');
    ...
    if (availableSlots <= 0) {
        throw httpError(400, 'Watchlist already contains the maximum of 10 stocks.');
    }
    ...
    if (newStocks.length > availableSlots) {
        throw httpError(400, `Only ${availableSlots} slot(s) available. Cannot add ${newStocks.length} stocks.`);
    }
...
module.exports.deleteStock = async ({ watchlistId, stockId, userId }) => {
    const result = await watchlistModel.findOneAndUpdate(
        ...
        { $pull: { stocks: { _id: new mongoose.Types.ObjectId(stockId) } } }, // ✅ cast to ObjectId
        { new: true }
    );
...
module.exports.updateWatchlist = async ({ id, watchlistName, stocks }) => {
    ...
    if (Array.isArray(stocks) && stocks.length > 0) {
        for (const stock of stocks) {
            ...
            if (stockSymbol) fields['stocks.$.stockSymbol'] = stockSymbol.trim().toUpperCase();

            if (Object.keys(fields).length > 0) {
                bulkOps.push({
                    updateOne: {
                        filter: { _id: id, 'stocks._id': new mongoose.Types.ObjectId(stockId) },
                        update: { $set: fields },
                    },
                });
            }
