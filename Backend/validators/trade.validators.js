...
module.exports.validateCreateTrade = [
    ...
    body('date')
        .isISO8601().toDate().withMessage('A valid entry date is required.'),
...
module.exports.validateCloseTrade = [
    body('closePrice')
        .isFloat({ min: 0.01 }).withMessage('Close price must be greater than 0.'),
...
module.exports.validateUpdateTrade = [
    ...
    body('entryPrice')
        .customSanitizer(cleanEmpty).optional()
        .isFloat({ min: 0.01 }).withMessage('Entry price must be greater than 0.'),

    body('entryDate')
        .customSanitizer(cleanEmpty).optional()
        .isISO8601().toDate().withMessage('Entry date must be a valid date.'),

    // ─── Immutable fields — reject loudly so client knows immediately ─────────
    body('openQty').not().exists().withMessage('openQty cannot be updated manually.'),
    body('closedQty').not().exists().withMessage('closedQty cannot be updated manually.'),
    body('remainingQty').not().exists().withMessage('remainingQty cannot be updated manually.'),
    body('realizedPnL').not().exists().withMessage('realizedPnL cannot be updated manually.'),
    body('status').not().exists().withMessage('status cannot be updated manually.'),
    body('closedAt').not().exists().withMessage('closedAt cannot be updated manually.'),
    body('avgExitPrice').not().exists().withMessage('avgExitPrice cannot be updated manually.'),
    body('lastExitDate').not().exists().withMessage('lastExitDate cannot be updated manually.'),
];